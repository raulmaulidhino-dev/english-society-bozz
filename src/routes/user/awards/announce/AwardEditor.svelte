<script lang="ts">
    ...
    let { userProfile, awardCategories, awardLevels, awardTitles, studentClasses }: Props = $props();
    ...
    let awardName: string = $state("");
    let awardCategoryId: string = $state("");
    let awardLevelId: string = $state("");
    let awardTitleId: string = $state("");
    let awardDate: string = $state(new Date().toISOString().split('T')[0]);
    let organizer: string = $state("");
    let organizerUrl: string | null = $state(null);
    let studentName: string = $state("");
    let studentClassId: string = $state("");
    let certificateUrl: string | null = $state(null);
    ...
    const addAward = async (token: string | null) => {
        ...
        try {
            const res = await axios.post(`${BACKEND_URL}/user/awards`,
                    awardDataReq,
                    {
                        headers: { 
        ...
        <section class="w-full rounded-t-[inherit] aspect-square relative">
            <button onclick={() => showUploadPhotoModal = true} class="w-full rounded-t-[inherit] aspect-square relative">
                {#if originalImage && showPhotoPreview}
                    <img src={originalImage} alt={awardName}
            ...
            <section class="p-4">
                ...
                hover:text-primary hover:bg-secondary">ANNOUNCE AWARD</button>
            </section>
        </section>
    </article>
</section>
