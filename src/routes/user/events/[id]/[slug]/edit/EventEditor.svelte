<script lang="ts">
    ...
    let eventImageUrl: string | null = $state(event?.image_url ?? null);
...
<Modal show={showUploadImageModal}>
    {#snippet children()}
        ...
        <button onclick={openFilePicker} class={`text-gray-300 w-full aspect-[3/2] h-auto bg-white border-2 border-gray-300 rounded-lg mx-auto flex justify-center items-center`}>
            ...
            {:else if eventImageUrl}
                <img src={eventImageUrl} alt="Event Thumbnail" class="mx-auto w-full rounded-[6px]" />
            {:else}
        ...
        <div class="flex justify-stretch items-stretch gap-2">
            ...
            <button id="deleteBtn" class={`grow text-white text-xs font-bold bg-red-700 border-2 border-red-700 hover:bg-red-900 p-2 rounded-full flex justify-center items-center gap-2 ${ originalImage || eventImageUrl ? "" : "hidden" }`}>
