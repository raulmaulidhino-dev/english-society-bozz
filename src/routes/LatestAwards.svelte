...
<section class="bg-secondary px-6 py-[4rem]">
    ...
    <section class="grid grid-cols-[repeat(auto-fill,_minmax(300px,_1fr))] gap-4">
        {#if awards}
            ...
            {/if}
        {:else}
            <section data-aos="zoom-in" class="bg-white pt-16 pb-8 px-4 max-w-xl rounded-2xl mx-auto shadow-lg col-span-full justify-self-center place-self-center center">
                <div class="mx-auto px-4 max-w-screen-sm text-center flex flex-col justify-center items-center">
                    ...
                    <button onclick={() => location.reload()} class="text-sm text-white text-center bg-primary font-bold w-full sm:w-fit border-2 border-primary py-2 px-2 md:px-8 rounded-full hover:text-primary hover:bg-secondary inline-block">REFRESH PAGE</button>
                </div>
            </section>        
        {/if}
    </section>
